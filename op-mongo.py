...
import kubernetes 
import yaml 
@kopf.on.create('dev.io', 'v1', 'mongo') 
def create_fn(body, spec, **kwargs): 
    ...
    if not nodeport: 
      ...
      # Pod template 
    pod = {'apiVersion': 'v1', 'metadata': {'name' : name, 'labels': {'app': 'mongo'}},'spec': {'containers': [ { 'image': image, 'name': name }]}} 
 
    # Service template 
    svc = {'apiVersion': 'v1', 'metadata': {'name' : name}, 'spec': { 'selector': {'app': 'mongo'}, 'type': 'NodePort', 'ports': [{ 'port': port, 'targetPort': port,  'nodePort': nodeport }]}} 
...
def delete(body, **kwargs): 
    msg = f"mongo {body['metadata']['name']} and its Pod / Service children deleted" 
    return {'message': msg} 